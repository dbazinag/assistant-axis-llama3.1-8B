#!/usr/bin/env python3
"""
collect_all_layers_activations.py

Forward-only activation collector that saves the pre-generation last-token
hidden state at EVERY transformer layer (not just 16/28), for the same
datasets/prompts already used by the layer-16 pipeline.

Mirrors collect_rtv_activations.py: reconstructs the exact prompts used by
the existing attack-family runs from their saved test_cases.json /
responses.jsonl, checks pair_id/behavior_id/attack_type alignment against
saved rows, and saves only prompt-side hidden states. It does not generate,
judge, or rescore — responses/labels are reused as-is from the existing runs.

Output format matches collect_harmbench_activations.py / run_all_traits_sweep_v2.py:
  {dataset}_rows.jsonl        — one row per pair (metadata + jailbroken label)
  {dataset}_activations.pt    — {pair_id: {"0": Tensor[4096], ..., "31": Tensor[4096]}}
"""
...
OUTPUT_ROOT = "full_trait_output/all_layers_activations"
OUTPUT_ROOT_OLMO3 = "full_trait_output/all_layers_activations_olmo3"
...
def process_row(model, tokenizer, row: dict, device: torch.device, n_layers: int):
    ...
    # Pre-generation last-token position, matching collect_harmbench_activations.py.
    layer_acts = {}
    for layer_idx in range(n_layers):
        hidden = outputs.hidden_states[layer_idx + 1][0, -1, :].detach().cpu()
        layer_acts[str(layer_idx)] = hidden
    return layer_acts
...
def worker_fn(worker_id: int, gpu_id: int, rows: List[dict], model_name: str, output_dir: Path, dataset: str):
    ...
    for row in tqdm(rows, desc=f"{dataset}/worker-{worker_id}", position=worker_id):
        try:
            activations[row["pair_id"]] = process_row(model, tokenizer, row, device, n_layers)
...
def main():
    ...
    manifest = {
        ...
        "model": args.model,
        "layers": "all (0..num_hidden_layers-1), pre-generation last token",
        "datasets": datasets,
        "test_mode": args.test,
        "notes": "Forward-only cache; keyed by str(layer_idx) where hidden_states[layer_idx + 1] is the layer output.",
    }
