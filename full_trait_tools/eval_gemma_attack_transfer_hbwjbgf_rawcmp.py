#!/usr/bin/env python3
"""HB+WJB+GPTFuzz-pooled transfer: RAW-5376 vs TRAIT-240.

Adds GPTFuzz to the in-distribution training mix, so transfer is tested on the
remaining held-out families the detector never saw: PAIR, PAP, PEZ.
"""
...
def main() -> None:
    ...
    ap.add_argument("--out", default=f"{fo}/attack_transfer_hbwjbgf_rawcmp_results.json")
    args = ap.parse_args()
    ...
    X_gf, y_gf = load_raw_xy(args.gptfuzz_acts, args.gptfuzz_clf)
    X_tr = np.concatenate([X_hb, X_wjb, X_gf], 0)
    y_tr = np.concatenate([y_hb, y_wjb, y_gf], 0)
    print(f"TRAIN HB+WJB+GPTFuzz raw X {X_tr.shape}  pos={int(y_tr.sum())}  base={y_tr.mean():.3f} "
          f"(HB pos={int(y_hb.sum())} | WJB pos={int(y_wjb.sum())} | GF pos={int(y_gf.sum())})", flush=True)

    tests = [
        ("HB+WJB+GF->PAIR", args.pair_acts, args.pair_clf),
        ("HB+WJB+GF->PAP", args.pap_acts, args.pap_clf),
        ("HB+WJB+GF->PEZ", args.pez_acts, args.pez_clf),
    ]
    ...
    for name, a, c in tests:
        ...
        print(f"{name:18s} {b:6.3f} | {raw_auc:12.3f} {raw_ap:6.3f} | {tr_auc:13.3f} {tr_ap:6.3f}", flush=True)

    avg = {fs: {m: sum(v[fs][m] for v in results.values()) / len(results) for m in ("auc", "ap")}
           for fs in ("raw5376", "trait240")}
    avg["base_rate"] = sum(v["base_rate"] for v in results.values()) / len(results)
    results["AVERAGE"] = avg
    print(f"{'AVERAGE':18s} {avg['base_rate']:6.3f} | {avg['raw5376']['auc']:12.3f} {avg['raw5376']['ap']:6.3f} | "
          f"{avg['trait240']['auc']:13.3f} {avg['trait240']['ap']:6.3f}", flush=True)
