...
def main() -> None:
    ...
    y_tr = Xy["HB"][1]
    P_tr = X_tr @ T.T
    print(f"TRAIN HB-only n={len(y_tr)} pos={int(y_tr.sum())} base={y_tr.mean():.3f}\n", flush=True)
    ...
    Path(f"{FO}/attack_transfer_hb_only_bootstrap_results.json").write_text(json.dumps(out, indent=2))
    print(f"\nSaved {FO}/attack_transfer_hb_only_bootstrap_results.json", flush=True)
