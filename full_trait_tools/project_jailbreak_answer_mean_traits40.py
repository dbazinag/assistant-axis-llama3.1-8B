#!/usr/bin/env python3
# Interview note: project old jailbreak mean-response activations onto the new traits40 assistant axis and all trait vectors, saving outputs under full_trait_output/.
...
DEFAULT_OUT_DIR = Path("full_trait_output/jailbreak_projections_traits40_answer_mean")
...
def main():
    ...
    if "mean_response_acts" not in sample:
        raise KeyError("Activation files must contain 'mean_response_acts'")

    answer_mean_sample = sample["mean_response_acts"].float()
    n_layers = answer_mean_sample.shape[0]
    hidden_size = answer_mean_sample.shape[1]
    ...
    run_meta = {
        ...
        "activation_source": "mean_response_acts",
        "output_activation_name": "answer_mean",
    }
    ...
    def add_to_group(
        container: dict,
        group_name: str,
        answer_mean_proj: list[float],
        answer_mean_cos: list[float],
    ):
        init_group_slot(container, group_name)
        slot = container[group_name]
        slot["count"] += 1
        for i in range(n_layers):
            slot["answer_mean_proj_sum"][i] += answer_mean_proj[i]
            slot["answer_mean_cos_sum"][i] += answer_mean_cos[i]
    ...
    with open(summary_jsonl, "w", encoding="utf-8") as f_out:
        for idx, pt_path in enumerate(act_files, start=1):
            try:
                ...
                axis_answer_mean_proj = dot_projection_per_layer(answer_mean_acts, axis)
                axis_answer_mean_cos = cosine_per_layer(answer_mean_acts, axis)
                ...
                for trait_name, trait_vec in trait_vectors.items():
                    ...
                    t_answer_mean_proj = dot_projection_per_layer(answer_mean_acts, trait_vec)
                    t_answer_mean_cos = cosine_per_layer(answer_mean_acts, trait_vec)

                    trait_out[trait_name] = {
                        "answer_mean_proj_all_layers": t_answer_mean_proj,
                        "answer_mean_cos_all_layers": t_answer_mean_cos,
                        "answer_mean_proj_mean": mean_val(t_answer_mean_proj),
                        "answer_mean_cos_mean": mean_val(t_answer_mean_cos),
                    }
                ...
                add_to_group(
                    aggregate["axis"],
                    prompt_label,
                    axis_answer_mean_proj,
                    axis_answer_mean_cos,
                )
                ...
                row = {
                    ...
                    "activation_source": "mean_response_acts",
                    "axis": {
                        "answer_mean_proj_all_layers": axis_answer_mean_proj,
                        "answer_mean_cos_all_layers": axis_answer_mean_cos,
                        "answer_mean_proj_mean": mean_val(axis_answer_mean_proj),
                        "answer_mean_cos_mean": mean_val(axis_answer_mean_cos),
                    },
                    "traits": trait_out,
                }
