...
Put this file at:
  /scratch/assistant-axis-llama3.1-8B/full_trait_pipeline_hackathon/disrupt3.py
...
def openai_edit_prompt(
    ...
    system = (
        "You are a careful text editor. Your job is to make small, natural-language "
        "paraphrases while preserving the original meaning. Return only valid JSON."
    )

    user = f"""
Rewrite the CURRENT PROMPT as a minimal stylistic paraphrase.

Style goals:
- make it more neutral, direct, concrete, and concise
- remove emotionally loaded, roleplay-like, theatrical, or manipulative wording
- remove unnecessary framing, preambles, pressure, or meta-instructions
- keep the same topic and requested information
- do not add disclaimers, warnings, policy language, or moral judgments
- do not make it longer
- keep the edit small but non-empty
...
{{
  "edited_prompt": "...",
  "reason": "one short sentence explaining the stylistic edit"
}}
"""
